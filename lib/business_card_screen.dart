                ...
                Column(
                  ...
                  children: [
                    ...
                    if (card.additionalInfo != null &&
                        card.additionalInfo!.isNotEmpty)
                      Padding(
...
class ExpandedCardDialog extends StatelessWidget {
  ...
  Future<void> _saveToContacts(BuildContext context, CardModel card) async {
    try {
      ...
      if (!await FlutterContacts.requestPermission()) {
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Contacts permission is required to save this contact',
              ),
      ...
      if (card.email != null && card.email!.isNotEmpty) {
        newContact.emails = [Email(card.email!, label: EmailLabel.work)];
      }

      // Add phone if available
      if (card.phone != null && card.phone!.isNotEmpty) {
        // Handle multiple phone numbers
        List<String> phoneNumbers = card.phone!
            .split(RegExp(r'[,;]'))
            .map((e) => e.trim())
      ...
      // Add additional info as notes
      if (card.additionalInfo != null && card.additionalInfo!.isNotEmpty) {
        newContact.notes = [Note(card.additionalInfo!)];
      }
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Could not launch email app. Please check if you have Gmail or an email app installed.',
            ),
          ),
        );
      }
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Could not open website: $url. Please check your internet connection.',
            ),
          ),
        );
      }
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          AspectRatio(
                            aspectRatio:
                                1.586 *
                                1.15, // Reduced height by 15% (increased aspect ratio)
                            child: Container(
                              decoration: BoxDecoration(
                                color: card.color,
                                borderRadius: BorderRadius.circular(16),
                          ...
                          Row(
                            children: [
                              Expanded(
                                child: GestureDetector(
                                  onTap: () => _saveToContacts(context, card),
                                  child: Container(
                                    height: 50,
                                    decoration: BoxDecoration(
...
class _BusinessCardScreenState extends State<BusinessCardScreen> {
  ...
  void _shareCard(CardModel card) {
    ...
    Share.share(cardInfo, subject: 'Business Card - ${card.name ?? 'Contact'}');
  }
  ...
  Future<void> _deleteCard(String cardId) async {
    try {
      ...
      final response = await http.post(
        ...
        body: json.encode({'card_id': cardId, 'action': 'inactive'}),
      );
                                ...
                                onTap: () {
                                  setState(() {
                                    ...
                                    _websiteController.text =
                                        cardWithColor.website ?? '';
                                    _addressController.text =
                                        cardWithColor.address ?? '';
                                  });
                                },
                                onShare: _shareCard,
                              ),
              ...
              child: _expandedCard != null
                  ? ExpandedCardDialog(
                      ...
                      onToggleEdit: () => setState(
                        () => _isDetailsEditMode = !_isDetailsEditMode,
                      ),
                      onSave: () {
                        ...
                        setState(() {
                          ...
                          final idx = _businessCards.indexWhere(
                            (c) => c.cardId == updated.cardId,
                          );
                          if (idx != -1) {
